# dags/ouchhungry_sales_pipeline.py
...
def extract_and_push(**kwargs):
    """
    Função para extrair dados do BigQuery e empurrá-los para o XCom.
    """
...
def notify_failure(context):
    """
    Função de callback para notificar em caso de falha no DAG.
    """
    email = ['correa.igor@aiqfome.com', 'diego.pastrello@aiqfome.com', 'leonardo.artur@aiqfome.com']
    subject = "Falha no Pipeline OuchHungry"
    html_content = f"""
    ...
    """
    logging.error("Falha no DAG. Notificando a equipe.")

def notify_success(**kwargs):
    """
    Função para notificar a equipe de vendas sobre o sucesso do pipeline.
    """
    logging.info("Pipeline concluído com sucesso. Notificando a equipe de vendas.")

with DAG(
    'ouchhungry_sales_pipeline',
    default_args=default_args,
    description='Pipeline modular para processamento de dados de vendas da OuchHungry',
    ...
    with TaskGroup("loading", tooltip="Carregamento de Dados") as loading_group:
        send_to_sftp_task = PythonOperator(
            ...
            op_kwargs={
                'df_json': "{{ ti.xcom_pull(task_ids='transformation.transform_transactions') }}",
                'sftp_hook': SFTPHook(ftp_conn_id='sftp_ouchhungry'),
                'remote_path': '/path/to/report.csv',
